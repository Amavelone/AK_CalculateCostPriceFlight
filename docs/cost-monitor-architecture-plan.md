...
2. В локальном MVP тот же черновик с небольшой задержкой сохраняется на сервере в JSON-хранилище. Перед производственным запуском этот adapter заменяется PostgreSQL, сохраняя защиту от очистки браузера, сбоя устройства и обновления приложения.
...
4. Все общие сущности — версии источников, распарсенные таблицы, ручные услуги и пути источников — хранятся на сервере с первого этапа: в JSON для локального MVP и во внешнем PostgreSQL после перехода к production.
